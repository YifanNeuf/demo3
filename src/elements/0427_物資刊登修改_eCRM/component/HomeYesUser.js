...
function Task({ id, email, level }) {
  ...
  useEffect(() => {
    ...
    onSnapshot(q, (querySnapshot) => {
      ...
      const recordIDs = donateRecordTmp.map(record => record.donateList.map(item => item.charity));
  ...
  const [charityData2, setCharityData2] = useState([
    {id: "001", name: "財團法人普仁青年關懷基金會", photo: "https://www.post.gov.tw/post/FileCenter/post_ww2/post_docdata_att/att_content/B396C802-0F3C-461A-BFAA-5C77307E3122/D243C0EB-23C6-4E7F-AFE3-DECBDA5D243F.jpg"},
    {id: "002", name: "財團法人高雄市私立星星兒社會福利基金會", photo: "https://www.starfamily.org.tw/assets/upload/adver/30.webp"}
  ])
  ...
  useEffect(() => {
    ...
    if (donateRecord) {
      ...
      donateRecord.forEach((name) => {
        // console.log(id);
        const docRef = query(collection(db, "charity"), where("info.name", "==", name));
